...
from supervised_models.multimodal_model.model import MultiModalNN
...
def training_loop(model, criterion, optimizer, scheduler, 
    ...
    for epoch in epoch_pbar:
        ...
        for batch_idx, (tabular_data, text_data, targets) in enumerate(batch_pbar):
            ...
            if total_iterations % log_every_n_iterations == 0:
                ...
                with torch.no_grad():
                    for val_tabular_data, val_text_data, val_targets in val_pbar:
                        val_tabular_data = val_tabular_data.to(device)
                        val_text_data = {k: v.to(device) for k, v in val_text_data.items()}
                        val_targets = val_targets.to(device)
                        
                        val_outputs = model(val_tabular_data, val_text_data)
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, 
                        default="supervised_models/multimodal_model/training_logs", 
    ...
    # --------------------
    # Tokenize Text with XLM-RoBERTa-large Tokenizer
    # --------------------
    ...
    print(f"Tokenized Text Keys: {list(tokenized_texts.keys())} and each has shape: {[v.shape for v in tokenized_texts.values()]}")
    ...
    # Note: tokenized_texts is a dict; apply indexing for each tensor.
    tokenized_texts = {k: v[indices] for k, v in tokenized_texts.items()}
    ...
    train_dataset = MultimodalDataset(
        ...
        tokenized_text={k: v[:train_samples] for k, v in tokenized_texts.items()},
        targets=targets[:train_samples]
    )
    val_dataset = MultimodalDataset(
        tabular_data=tabular_data_matrix[train_samples:train_samples+val_samples],
        tokenized_text={k: v[train_samples:train_samples+val_samples] for k, v in tokenized_texts.items()},
    ...
    # --------------------
    # Note: The MLP model now internally uses a pretrained XLM-RoBERTa-large
    # and expects tokenized text inputs (a dict) in its forward method.
    model = MultiModalNN(
